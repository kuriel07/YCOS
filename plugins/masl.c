...
uchar masl_decode(void) _REENTRANT_ {			 //return response length	 
	...
	if(_select(&temp_fs, FID_WIB) < 0x9F00) goto exit_plugin;
	if(_select(&temp_fs, FID_WSMSHDRLIST) < 0x9F00) goto exit_plugin;
	_readrec(&temp_fs, vas_cistr[1], STK_buffer, 27); 
	if(_select(&temp_fs, FID_WSMSHEADER) < 0x9F00) goto exit_plugin;	 
	_writebin(&temp_fs, 0, STK_buffer, 27);
